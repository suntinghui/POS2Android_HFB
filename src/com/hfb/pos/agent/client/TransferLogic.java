package com.hfb.pos.agent.client;
...
import com.hfb.pos.activity.AnnouncementListActivity;
import com.hfb.pos.activity.AuthenticationInfoActivity;
import com.hfb.pos.activity.BankSearchActivity;
import com.hfb.pos.activity.BaseActivity;
import com.hfb.pos.activity.DrawMoneyActivity;
import com.hfb.pos.activity.FaileActivity;
import com.hfb.pos.activity.FindPwdModifyActivity;
import com.hfb.pos.activity.LRCatalogActivity;
import com.hfb.pos.activity.SettlementSuccessActivity;
import com.hfb.pos.activity.TimeoutService;
import com.hfb.pos.activity.TransferDetailListActivity;
import com.hfb.pos.agent.client.db.TransferSuccessDBHelper;
import com.hfb.pos.agent.client.db.UploadSignImageDBHelper;
...
import com.hfb.pos.fsk.FSKOperator;
import com.hfb.pos.model.AnnouncementModel;
import com.hfb.pos.model.BankModel;
import com.hfb.pos.model.FieldModel;
import com.hfb.pos.model.TransferDetailModel;
import com.hfb.pos.model.TransferModel;
import com.hfb.pos.model.TransferSuccessModel;
import com.hfb.pos.model.UserModel;
import com.hfb.pos.util.AssetsUtil;
import com.hfb.pos.util.PhoneUtil;
...
public class TransferLogic {
	...
	private void loginDone(HashMap<String, String> fieldMap) {
		...
		if ("1".equals(fieldMap.get("respmsg"))) {
			...
			// 登陆成功，跳转到菜单界面
			BaseActivity.getTopActivity().startActivity(new Intent("com.hfb.pos.lrcatalog"));
...
//					JSONArray jsonArray = (JSONArray) parse.nextValue();
	...
	private void queryHistoryGroupDone(HashMap<String, String> fieldMap) {
		...
		if (null != sendMap && sendMap.containsKey("BeginDate") && sendMap.containsKey("EndDate")) {
			Intent intent = new Intent("com.hfb.pos.queryTransferHistoryGroupActivity");
	...
	private void queryHistoryListDone(HashMap<String, String> fieldMap) {
		...
		if (null != sendMap && sendMap.containsKey("totalCount")) {
			Intent intent = new Intent("com.hfb.pos.queryTransferHistoryList");
	...
	private void receiveTransDone(HashMap<String, String> fieldMap) {
		...
		try {
			if(Constant.isAISHUA){
				...
				if (AppDataCenter.getValue("__TERSERIALNO").startsWith("001917")) { 
					// 打印
					Intent intent = new Intent("com.hfb.pos.PrintReceiptActivity");
		 ...
		 * return; } }
		 * 
		 * // 打印 Intent intent = new Intent("com.hfb.pos.printReceipt");
		 * intent.putExtra("content", fieldMap);
		 * BaseActivity.getTopActivity().startActivityForResult(intent, 0);
...
//			map.put("batchNum", fieldsMap.get("field60").substring(2, 8));
			...
			try {
				BaseActivity.getTopActivity().startService(new Intent("com.hfb.pos.uploadSignImageService"));
	...
	private void checkUpdateAPK(HashMap<String, String> fieldMap) {
		Intent intent = new Intent("com.hfb.pos.updateAPKService");
